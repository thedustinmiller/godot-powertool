...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Web(web_cmd) => match web_cmd {
            WebCommands::Setup => web::cmd_setup(),
            WebCommands::Dev { port } => web::cmd_dev(port),
            WebCommands::Build => web::cmd_build(),
            WebCommands::Export { release } => web::cmd_export(release),
            WebCommands::Test {
                headed,
                ui,
                debug,
                filter,
            } => web::cmd_test(headed, ui, debug, filter.as_deref()),
            WebCommands::Preview { port } => web::cmd_preview(port),
            WebCommands::Doctor => web::cmd_doctor(),
        },
